...
void main() {
  ...
  if(uMouseEnabled > 0.5) {
     vec2 mp = iMouse.xy / iResolution.y * ref;
...
export default function Ferrofluid(props: FerrofluidProps) {
  ...
  useEffect(() => {
    ...
    const { arr, count } = prepColors(props.colors || []);
    ...
    const uniforms = {
      ...
      uFlow: { value: flowVec(props.flowDirection || 'down') },
      uSpeed: { value: props.speed ?? 0.5 },
      uScale: { value: props.scale ?? 1.6 },
      uTurbulence: { value: props.turbulence ?? 1 },
      uFluidity: { value: props.fluidity ?? 0.1 },
      uRimWidth: { value: props.rimWidth ?? 0.2 },
      uSharpness: { value: props.sharpness ?? 2.5 },
      uShimmer: { value: props.shimmer ?? 1.5 },
      uGlow: { value: props.glow ?? 2 },
      uOpacity: { value: props.opacity ?? 1 },
      uMouseEnabled: { value: props.mouseInteraction ?? true ? 1 : 0 },
      uMouseStrength: { value: props.mouseStrength ?? 1 },
      uMouseRadius: { value: props.mouseRadius ?? 0.35 }
    };
    ...
    if (props.mouseInteraction ?? true) canvas.addEventListener('pointermove', onPointerMove);

    const loop = (t: number) => {
      rafRef.current = requestAnimationFrame(loop);
      uniforms.iTime.value = t * 0.001;

      const dampening = props.mouseDampening ?? 0.15;
      if (dampening > 0) {
      ...
      if (!props.paused && meshRef.current) renderer.render({ scene: meshRef.current });
    };
    rafRef.current = requestAnimationFrame(loop);

    return () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      if (props.mouseInteraction ?? true) canvas.removeEventListener('pointermove', onPointerMove);
      ro.disconnect();
  ...
  return <div ref={containerRef} className={`ferrofluid-container ${props.className ?? ''}`} style={{ mixBlendMode: props.mixBlendMode as any }} />;
}
